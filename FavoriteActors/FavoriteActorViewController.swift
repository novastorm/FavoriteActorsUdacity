...
class FavoriteActorViewController : UITableViewController, ActorPickerViewControllerDelegate {
    ...
    func actorPicker(actorPicker: ActorPickerViewController, didPickActor actor: Person?) {
        ...
        if let newActor = actor {
            ...
            actorDictionary[Person.Keys.ID] = newActor.id
            actorDictionary[Person.Keys.Name] = newActor.name
            actorDictionary[Person.Keys.ProfilePath] = newActor.imagePath
            
            // Create a new Person in the shared context
            let actor = Person(dictionary: actorDictionary, context: self.sharedContext)
